...
int setupGeometry(){
    const float raio = 0.8f;

    const float anguloInicio = 240.0f * 3.14 / 180.0f;
    const float anguloFim = 300.0f * 3.14 / 180.0f;

    // +2:
    // 1 vértice para o centro
    // + segmentos + 1 vértices para a borda
    GLfloat vertices[(segmentos + 2) * 3];
    ...
    for (int i = 0; i <= segmentos; i++)
    {
        ...
        float angulo = anguloInicio + t * (anguloFim - anguloInicio);
